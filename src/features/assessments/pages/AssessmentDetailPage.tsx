...
export default function AssessmentDetailPage() {
  ...
  const handleReaccess = async (submissionId: string) => {
    ...
    finally { setRegranting(null); }
  };

  const handleExport = async () => {
    setExporting(true);
    try {
      const res = await api.get(`/api/assessments/${id}/submissions/export`, { responseType: 'blob' });
  ...
  const openDetail = (sub: SubmissionWithCandidate) => { setSelected(sub); setShowDetail(true); };
    ...
    <div>
      <Header
        ...
        actions={
          <div style={{ display: 'flex', gap: 8 }}>
            <Button variant="secondary" leftIcon={<IconChevronLeft size={16} />} onClick={() => navigate(-1)}>
              Back
            </Button>
            <Button variant="secondary" leftIcon={<IconDownload size={16} />} onClick={handleExport} isLoading={exporting}>
              Export
            </Button>
          </div>
        }
      ...
      {isLoading ? (
        ...
        <div className={styles.empty}>
          ...
          <span style={{ fontSize: 12 }}>Candidates will appear here once they start the assessment</span>
        </div>
      ) : (
        <>
                ...
                {submissions.map((sub) => {
                  ...
                  return (
                    <tr key={sub.id}>
                      ...
                      <td style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Round {sub.current_round}</td>
                      <td style={{ fontSize: 13, color: 'var(--text-tertiary)' }}>{formatDateTime(sub.started_at)}</td>
                      <td>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button className={styles.actionBtn} onClick={() => openDetail(sub)} title="View Details">
                            <IconEye size={14} />
                          </button>
                          {(sub.status === 'completed' || sub.status === 'malpractice') && (
                            ...
                            >
                              {regranting === sub.id ? <Spinner size="sm" /> : <IconRefresh size={14} />}
                            </button>
                          )}
                        </div>
                      </td>
              ...
              <div className={styles.malpracticeBox}>
                ...
                {selected.malpractice_flags.map((flag, i) => (
                  <div key={i} className={styles.flagItem}>
                    <span>{flag.type}</span>
                    <span style={{ color: 'var(--text-tertiary)', fontSize: 11 }}>{formatDateTime(flag.flagged_at)}</span>
                  </div>
                ))}
              </div>
            )}

            {selected.rounds && selected.rounds.map((round, ri) => (
              <div key={ri} className={styles.roundSection}>
                <h4 style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, color: 'var(--text-secondary)' }}>
                  Round {round.round_number}
                </h4>
                {round.answers && Object.entries(round.answers).map(([qId, answer]) => (
                  <div key={qId} className={styles.answerItem}>
                    <p style={{ fontSize: 13, color: 'var(--text-primary)', marginBottom: 4 }}>Q: {qId}</p>
