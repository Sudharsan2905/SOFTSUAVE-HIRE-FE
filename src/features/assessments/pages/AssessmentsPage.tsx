...
import {
  IconPlus, IconEdit, IconDelete, IconShare, IconClone,
  IconUsers, IconAssessment, IconTime, IconMonitor, IconCopy, IconMail,
  IconWhatsApp, IconChevronRight,
} from '@/assets/icons';
import { api } from '@/utils/api';
import { useDebounce } from '@/hooks/useDebounce';
import { usePagination } from '@/hooks/usePagination';
import { Assessment, MonitoringConfig, PaginationMeta, ViewMode, SortOrder } from '@/types';
...
export default function AssessmentsPage() {
  ...
  const [cloneSource, setCloneSource] = useState<Assessment | null>(null);
  ...
  useEffect(() => { reset(); }, [debouncedSearch, sortBy, sortOrder]);
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <>
          <div className={viewMode === 'grid' ? styles.grid : styles.list}>
            {assessments.map((a) => (
              <div key={a.id} className={styles.card} onClick={() => navigate(`/workspaces/${workspaceId}/assessments/${a.id}`)}>
                <div className={styles.cardTop}>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <Badge variant={a.accessibility === 'monitoring' ? 'accent' : 'default'}>
                      {a.accessibility === 'monitoring' ? '🛡 Monitoring' : 'Normal'}
                    </Badge>
                    <Badge variant="info">{a.rounds.length} round{a.rounds.length !== 1 ? 's' : ''}</Badge>
                  </div>
                  <div className={styles.cardActions} onClick={(e) => e.stopPropagation()}>
                    <button className={styles.iconBtn} onClick={() => { setSelected(a); setShowShare(true); }} title="Share">
                      <IconShare size={14} />
                    </button>
                    <button className={styles.iconBtn} onClick={() => handleClone(a)} title="Clone">
                      <IconClone size={14} />
                    </button>
                    <button className={`${styles.iconBtn} ${styles.danger}`} onClick={() => { setSelected(a); setShowDelete(true); }} title="Delete">
                      <IconDelete size={14} />
                    </button>
                  </div>
                </div>
                ...
                <div className={styles.metaRow}>
                  <span className={styles.metaItem}><IconTime size={12} /> {a.rounds.reduce((t, r) => t + r.max_duration_minutes, 0)} min total</span>
                  <span className={styles.metaItem}><IconUsers size={12} /> {a.submission_count ?? 0} submitted</span>
                </div>

                <div className={styles.cardFooter}>
                  <span className={styles.dateText}>{formatDate(a.created_at)}</span>
                  <span className={styles.viewDetail}>View Details <IconChevronRight size={12} /></span>
                </div>
              </div>
            ))}
          </div>
      ...
      {showWizard && workspaceId && (
        <CreateAssessmentWizard
          ...
          onSuccess={() => { setShowWizard(false); fetchAssessments(); }}
        />
      )}

      {/* Share Modal */}
      <Modal isOpen={showShare} onClose={() => setShowShare(false)} title="Share Assessment" size="md">
        <div className={styles.shareCards}>
          <button
            className={styles.shareCard}
            onClick={() => { copyToClipboard(shareUrl).then(() => toast.success('Link copied!')); }}
          >
      ...
      <Modal isOpen={showDelete} onClose={() => setShowDelete(false)} title="Delete Assessment"
        ...
        <p style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
          Delete <strong>{selected?.name}</strong>? This will remove all candidate submissions. This action cannot be undone.
        </p>
      </Modal>
    </div>
  );
